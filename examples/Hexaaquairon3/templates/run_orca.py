...
for i, line in enumerate(flines):
    if 'HIRSHFELD ANALYSIS' in line:
        ...
        atomic_charges = []
        continue
    if getchrg and skiplines > 0:
        skiplines -= 1
    elif getchrg and len(line.split()) == 0:
        getchrg = False
    elif getchrg:
        chrg_i = float(line.split()[-2])
        atomic_charges.append(chrg_i)
results['atomic_charges'] = list(atomic_charges)
